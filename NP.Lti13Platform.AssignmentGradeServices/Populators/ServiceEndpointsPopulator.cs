    ...
    public override async Task PopulateAsync(IServiceEndpoints obj, MessageScope scope, CancellationToken cancellationToken = default)
    {
        ...
        if (lineItemScopes.Count > 0 && scope.Context != null)
        {
            ...
            obj.ServiceEndpoints = new IServiceEndpoints.LineItemServiceEndpoints
            {
                ...
                LineItemsUrl = linkGenerator.GetUriByName(RouteNames.GET_LINE_ITEMS, new { deploymentId = scope.Deployment.DeploymentId, contextId = scope.Context.ContextId }, config.ServiceAddress.Scheme, new HostString(config.ServiceAddress.Authority)),
                LineItemUrl = string.IsNullOrWhiteSpace(lineItemId) ? null : linkGenerator.GetUriByName(RouteNames.GET_LINE_ITEM, new { deploymentId = scope.Deployment.DeploymentId, contextId = scope.Context.ContextId, lineItemId }, config.ServiceAddress.Scheme, new HostString(config.ServiceAddress.Authority)),
            };
        }
    }
}
